...
use oauth2::{basic::{BasicClient, BasicTokenType}, reqwest::async_http_client, AuthorizationCode, CsrfToken, EmptyExtraTokenFields, RedirectUrl, Scope, StandardTokenResponse};
use url::Url;
...
pub async fn request_token(basic_client: &BasicClient, options: RequestTokenOptions) -> Result<StandardTokenResponse<EmptyExtraTokenFields, BasicTokenType>, RequestTokenError> {
    let client = check_redirect(&basic_client, &options.port, &options.host, &options.redirect_path)?;
    let (auth_url, csrf_token) = get_auth_url(&client, &options.scopes);
    ...
    let handler: tokio::task::JoinHandle<()> = tokio::spawn(async move {
        ...
        let app = Router::new()
            ...
            .with_state(AppState{ state_token: csrf_token, client: client_clone, sender: sender.clone()});

        let listener = match tokio::net::TcpListener::bind(format!("{}:{}", &options.host, &options.port)).await{
            Ok(l) => l,
            Err(e) => {
                send_message(sender.clone(), Message::OtherError(e.to_string()));
                return ;
    ...
    let token =loop {
        ...
        match message {
            ...
            Message::ApiError(e) => {
                return Err(RequestTokenError::ApiError(e))
            }
            Message::OtherError(e) => {
                return Err(RequestTokenError::Other(e))
            },
        }
    };
...
fn get_auth_url(basic_client: &BasicClient, scopes: &Vec<String>) -> (Url, CsrfToken) {
    let scopes: Vec<Scope> = scopes.into_iter().map(|s| Scope::new(s.to_owned())).collect();
...
fn check_redirect(basic_client: &BasicClient, port: &u16, host: &str, redirect_path: &str) -> Result<BasicClient, RequestTokenError> {
